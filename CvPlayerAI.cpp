...
int CvPlayerAI::AI_movementPriority(CvSelectionGroup* pGroup) const
{
	...
	if (pHeadUnit != NULL)
	{
		...
		{
//			if (pHeadUnit->withdrawalProbability() > 20) Vincentz
			if (pHeadUnit->withdrawalProbability() > 30)
			{
				return 7;
			}
...
bool CvPlayerAI::AI_captureUnit(UnitTypes eUnit, CvPlot* pPlot) const
{
	...
	if (pNearestCity != NULL)
	{
//		if (plotDistance(pPlot->getX_INLINE(), pPlot->getY_INLINE(), pNearestCity->getX_INLINE(), pNearestCity->getY_INLINE()) <= 4) Vincentz
		if (plotDistance(pPlot->getX_INLINE(), pPlot->getY_INLINE(), pNearestCity->getX_INLINE(), pNearestCity->getY_INLINE()) <= 20)
		{
			return true;
		}
	}
...
/*************************************************************************************************/
		...
		for (iI = 0; iJ < GC.getNumSpecialistInfos(); iI++)
			{
				for (iJ = 0; iJ < NUM_COMMERCE_TYPES; iJ++)
					{
				...
				if (pLoopUnit->getUnitInfo().getProductionCost() > 0)
				{
					if ((iExpThreshold == -1) || (pLoopUnit->canFight() && pLoopUnit->getExperience() <= iExpThreshold))
					{
						if (!(pLoopUnit->isMilitaryHappiness()) || !(pLoopUnit->plot()->isCity()) || (pLoopUnit->plot()->plotCount(PUF_isMilitaryHappiness, -1, -1, getID()) > 2))
						{
							...
							//@MOD GFD: AI will disband units inside cultural borders more likely.
