...
CvUnit::CvUnit()
{
	...
	*
	****************************************/
	m_paiSeeInvisible = NULL;
	/**
	** End: SeeInvisiblePromotion
	**/
	//Vincentz
...
void CvUnit::uninit()
{
	...
	*
	****************************************/
	SAFE_DELETE_ARRAY(m_paiSeeInvisible);
	/**
	** End: SeeInvisiblePromotion
	**/
...
void CvUnit::reset(int iID, UnitTypes eUnit, PlayerTypes eOwner, bool bConstructorCall)
{
	...
	*
	****************************************/
	m_paiSeeInvisible = new int[GC.getNumInvisibleInfos()];
	for (iI = 0; iI < GC.getNumInvisibleInfos(); iI++)
	{
		m_paiSeeInvisible[iI] = 0;
	}
	/**
	** End: SeeInvisiblePromotion
	**/
...
void CvUnit::resolveCombat(CvUnit* pDefender, CvPlot* pPlot, CvBattleDefinition& kBattle)
{
	...
	if (pPlot->getNumDefenders(pDefender->getOwner()) == 1 && pDefender->baseMoves() + 2 > baseMoves())	//must be faster than attacker
	{
	...
	int WDModifier;
	int WDWWModifier;
	if (GET_PLAYER(this->getOwnerINLINE()).isNoNonStateReligionSpread()) { WDModifier = 2; }
	else { WDModifier = 1; }
	if (GET_PLAYER(this->getOwnerINLINE()).getWarWearinessModifier() > 50) { WDWWModifier = 5; }
	else { WDWWModifier = 1; }
	...
	while (true)
	{
		//Vincentz MaxCombatRounds
		iMaxCombatRounds++;
		if (iMaxCombatRounds > GC.getDefineINT("MAX_COMBATROUNDS") + movesLeft()/GC.getDefineINT("MOVE_DENOMINATOR")) 
		{
			...
			//if (GC.getGameINLINE().getSorenRandNum(GC.getDefineINT("COMBAT_DIE_SIDES"), "Combat") < 400 + iDefenderOdds / 5) // Vincentz New Combat
		{
			if (getCombatFirstStrikes() == 0)
			{
				...
				if (getDamage() + iAttackerDamage * WDWWModifier >= maxHitPoints() && GC.getGameINLINE().getSorenRandNum(100, "Withdrawal") < withdrawalProbability() / WDModifier)
				{
		...
		else
		{
			if (pDefender->getCombatFirstStrikes() == 0)
			{
				...
				//defender trying to withdraw:
					...
					&& !isSuicide() && iCloseCombatRoundNum > 3)	//can not to escape at close combat round 1
				{
...
bool CvUnit::canLoadUnit(const CvUnit* pUnit, const CvPlot* pPlot) const
{
	...
	//Vincentz Load Unit if have moves
	if (!(canMove()) && !(pPlot->isWater()))
	{
		return false;
	}
	//Vincentz Load Unit if have moves end
	return true;
}
...
bool CvUnit::canHeal(const CvPlot* pPlot) const
{
	...
	//Vincentz Healing
	if (getGroup()->getActivityType() != ACTIVITY_HEAL && isWaiting())
		//	if (isWaiting())
	{
		return false;
	}
	...
	// Vincentz Healing Cost
{
	...
	{
		if (GC.getGameINLINE().isOption(GAMEOPTION_HEALING_COST_GOLD))
		{
			...
			{
				//			CvWString szBuffer = gDLL->getText("TXT_KEY_HEALING_COST", healRate(plot()), getUnitInfo().getProductionCost(), getDamage(), GC.getDefineINT("UNITHEAL_COSTMODIFIER"), unitHealCost, unitHealCost+1);
				//			gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_WONDER_UNIT_BUILD", MESSAGE_TYPE_MAJOR_EVENT, getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_UNIT_TEXT"), getX_INLINE(), getY_INLINE(), true, true);
...
bool CvUnit::paradrop(int iX, int iY)
{
	...
	changeMoves(GC.getMOVE_DENOMINATOR() * GC.getDefineINT("MOVE_MULTIPLIER") * 2);
	//	setMadeAttack(true); Vincentz Paratrooper
...
bool CvUnit::canAirBombAt(const CvPlot* pPlot, int iX, int iY) const
{
	...
	{
		//Vincentz Cannot bombard negative improvement costs
		if (pTargetPlot->getImprovementType() == NO_IMPROVEMENT && pTargetPlot->getRouteType() == NO_ROUTE)
		{
			return false;
		}
...
bool CvUnit::airBomb(int iX, int iY)
{
	...
	if (pCity != NULL)
	{
		//Vincentz Randomized Airbomb
		pCity->changeDefenseModifier((-airBombCurrRate() * 50 + GC.getGameINLINE().getSorenRandNum(100, "Air Bomb - Random")) / 100);
	...
	setMadeAttack(true);
	//Vincentz Moves extra move for airbomb
	changeMoves(GC.getMOVE_DENOMINATOR() * 2);
...
bool CvUnit::bombard()
{
	...
	//Vincentz Bombard random
	int RandomBombardChance = GC.getGameINLINE().getSorenRandNum(100, "RandomHit");
	int RandomBombardDamage = 50 + GC.getGameINLINE().getSorenRandNum(100, "RandomDamage");
	...
	/*
	// < Bombard Experience Start >
	int iExperience = (int)((attackXPValue()*((float)pBombardCity->getDefenseModifier(false)/100))/1.25);
	iExperience = range(iExperience, GC.getDefineINT("MIN_EXPERIENCE_PER_COMBAT"), GC.getDefineINT("MAX_EXPERIENCE_PER_COMBAT"));
	changeExperience(iExperience, maxXPValue(), false, pPlot->getOwnerINLINE() == getOwnerINLINE(), !isBarbarian());
	// <Bombard Experience End   >
	*/
...
bool CvUnit::canPillage(const CvPlot* pPlot) const
{
	...
	//Vincentz Cannot pillage negative improvement cost
	if ((pPlot->getImprovementType() == NO_IMPROVEMENT) || (GC.getImprovementInfo(pPlot->getImprovementType()).getPillageGold() < 0))
	{
		if (!(pPlot->isRoute()))
		{
			return false;
...
bool CvUnit::pillage()
{
	...
	//Vincentz Cannot pillage negative improvement costs
	if ((pPlot->getImprovementType() != NO_IMPROVEMENT) && !(GC.getImprovementInfo(pPlot->getImprovementType()).getPillageGold() < 0))
	{
		...
		if (isEnemy((TeamTypes)iTeam))
		{
			for (int i = -iBlockadeRange; i <= iBlockadeRange; ++i)
			{
				for (int j = -iBlockadeRange; j <= iBlockadeRange; ++j)
				{
					...
					//					if (NULL != pLoopPlot && pLoopPlot->isWater() && pLoopPlot->area() == area()) Vincentz
					if (NULL != pLoopPlot && pLoopPlot->area() == area())
					{
						if (!bChanged)
						{
...
//TSHEEP start SuperSpy Vincentz
...
bool CvUnit::espionage(EspionageMissionTypes eMission, int iData)
{
	...
	{
		//Vincentz Spy
		if (testSpyIntercepted(eTargetPlayer, (GC.getEspionageMissionInfo(eMission).getDifficultyMod() - evasionProbability())))
			//Vincentz Spy
		{
			return false;
		}
		...
		if (GET_PLAYER(getOwnerINLINE()).doEspionageMission(eMission, eTargetPlayer, plot(), iData, this))
		{
			...
			//Vincentz Spy
			if (!testSpyIntercepted(eTargetPlayer, (GC.getDefineINT("ESPIONAGE_SPY_MISSION_ESCAPE_MOD") - evasionProbability())))
			{
				setFortifyTurns(0);
				setMadeAttack(true);
				//				finishMoves();
				changeMoves(GC.getMOVE_DENOMINATOR() * ((200 + GC.getEspionageMissionInfo(eMission).getDifficultyMod()) / 5));
				//TSHEEP Give spies xp for successful missions SuperSpy Vincentz
...
//Vincentz
int CvUnit::getUnitClassCount(UnitClassTypes eIndex) const
{
...
void CvUnit::upgrade(UnitTypes eUnit)
{
	...
	//TODO Vincentz upgrade takes 3 turns
	pUpgradeUnit->changeMoves(maxMoves() * 3);
	//pUpgradeUnit->finishMoves();
	//Vincentz Upgrade end
	...
	if (pUpgradeUnit->getLeaderUnitType() == NO_UNIT)
	{
		...
		if (pUpgradeUnit->getExperience() > MaxXPEra)
		{
...
int CvUnit::getNumSeeInvisibleTypes() const
{
	...
	** ---- End Original Code      ----   **/
	int numSeeInvisibles = 0;
	for (int i = 0; i < GC.getNumInvisibleInfos(); i++)
	{
		if (getSeeInvisibleCount((InvisibleTypes)i) > 0)
		{
			numSeeInvisibles++;
		}
	}
	return numSeeInvisibles;
	/**
	** End: SeeInvisiblePromotion
	**/
}
...
InvisibleTypes CvUnit::getSeeInvisibleType(int i) const
{
	...
	** ---- End Original Code      ----   **/
	InvisibleTypes eInvisibleType = NO_INVISIBLE;
	int currSeeVisibleIndex = 0;
	for (int j = 0; j < GC.getNumInvisibleInfos(); j++)
	{
		if (getSeeInvisibleCount((InvisibleTypes)j) > 0 && currSeeVisibleIndex++ == i)
		{
			return (InvisibleTypes)j;
		}
	}
	return eInvisibleType;
	/**
	** End: SeeInvisiblePromotion
	**/
}
...
int CvUnit::airCombatDamage(const CvUnit* pDefender) const
{
	...
	//Vincentz Random Damage Start
	iDamage *= (50 + GC.getGameINLINE().getSorenRandNum(100, "RandomHit"));
	iDamage /= 100;
	//Vincentz Random Damage End
...
void CvUnit::setHasPromotion(PromotionTypes eIndex, bool bNewValue)
{
	...
	if (isHasPromotion(eIndex) != bNewValue)
	{
		...
		*
		****************************************/
		bool bAdd = iChange == 1 ? true : false;
		std::vector<InvisibleTypes> vInvisibilityTypes;
		for (int i = 0; i < GC.getPromotionInfo(eIndex).getNumSeeInvisibleTypes(); i++)
		{
			changeSeeInvisibleCount((InvisibleTypes)GC.getPromotionInfo(eIndex).getSeeInvisibleType(i), iChange);
			vInvisibilityTypes.push_back((InvisibleTypes)GC.getPromotionInfo(eIndex).getSeeInvisibleType(i));
		}
		if (vInvisibilityTypes.size() > 0)
		{
			plot()->changeAdjacentSight(getTeam(), visibilityRange(), bAdd, this, true, vInvisibilityTypes);
		}
...
void CvUnit::read(FDataStreamBase* pStream)
{
	...
	*
	****************************************/
	pStream->Read(GC.getNumInvisibleInfos(), m_paiSeeInvisible);
	/**
	** End: SeeInvisiblePromotion
	**/
...
void CvUnit::write(FDataStreamBase* pStream)
{
	...
	*
	****************************************/
	pStream->Write(GC.getNumInvisibleInfos(), m_paiSeeInvisible);
	/**
	** End: SeeInvisiblePromotion
	**/
...
bool CvUnit::interceptTest(const CvPlot* pPlot)
{
	if (GC.getGameINLINE().getSorenRandNum(100, "Evasion Rand") >= evasionProbability())
	{
		...
		if (pInterceptor != NULL)
		{
			...
			if (GC.getGameINLINE().isOption(GAMEOPTION_BETTER_AIR_INTERCEPTION))
...
bool CvUnit::airStrike(CvPlot* pPlot)
{
	...
	setMadeAttack(true);
	//Vincentz Moves extra move for Airstrike
	changeMoves(GC.getMOVE_DENOMINATOR() * 2);
...
bool CvUnit::canRangeStrikeAt(const CvPlot* pPlot, int iX, int iY) const
{
	...
	*/
	...
	if (((plot()->isCity(true, getTeam())) || (pPlot->isHills())) && (getDomainType() == DOMAIN_LAND))
	{
...
bool CvUnit::rangeStrike(int iX, int iY)
{
	...
	{
		pCity->changeDefenseModifier((-bombardRate() * currHitPoints() / maxHitPoints() * 50 + GC.getGameINLINE().getSorenRandNum(100, "Bombard - Random")) / 100);
	...
	//Vincentz Rangestrike Strikeback
	//	if (pDefender->plotDistance(pPlot->getX_INLINE(), pPlot->getY_INLINE(), pTargetPlot->getX_INLINE(), pTargetPlot->getY_INLINE()) > airRange())
	//	if (GC.getDefineINT("RANGESTRIKE_RETURN_FIRE") == 1 && (pDefender->canRangeStrikeAt(pDefender->plot(), this->plot()->getX_INLINE(), this->plot()->getY_INLINE())))
	if (GC.getDefineINT("RANGESTRIKE_RETURN_FIRE") == 1)
	{
		if (!pDefender->canRangeStrikeAt(pDefender->plot(), this->plot()->getX_INLINE(), this->plot()->getY_INLINE()))
		{
			pDefender = NULL;
			...
			while (NULL != pUnitNode)
			{
				...
				if (pLoopUnit->canRangeStrikeAt(pLoopUnit->plot(), this->plot()->getX_INLINE(), this->plot()->getY_INLINE()))
				{
					pDefender = pLoopUnit;
				}
			}
		...
		if (pDefender != NULL)
		{
			...
			if (((GC.getGameINLINE().getSorenRandNum(GC.getDefineINT("RANGESTRIKE_DICE"), "Random")) + pDefender->airBaseCombatStr()) * GC.getDefineINT("RANGESTRIKE_HIT_MODIFIER") > ((GC.getGameINLINE().getSorenRandNum(GC.getDefineINT("RANGESTRIKE_DICE"), "Random")) + this->baseCombatStr()))
			{
				szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_RETURN_ATTACK_BY_AIR", pDefender->getNameKey(), getNameKey(), (((iUnitDamage - this->getDamage()) * 100) / this->maxHitPoints()));
				gDLL->getInterfaceIFace()->addMessage(pDefender->getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_COMBAT", MESSAGE_TYPE_INFO, getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), this->getX_INLINE(), this->getY_INLINE(), true, true);
				...
				gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_COMBAT", MESSAGE_TYPE_INFO, this->getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pPlot->getX_INLINE(), pPlot->getY_INLINE());

				this->setDamage(iUnitDamage, this->getOwnerINLINE(), false);
				//			collateralCombat(plot(), this);
			}
			else
			{
				szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_RETURN_ATTACK_BY_AIR_MISS", pDefender->getNameKey(), getNameKey());
				gDLL->getInterfaceIFace()->addMessage(pDefender->getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_COMBAT", MESSAGE_TYPE_INFO, getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), this->getX_INLINE(), this->getY_INLINE(), true, true);
...
bool CvUnit::isAlwaysHostile(const CvPlot* pPlot) const
{
	...
	return true;
	//Vincentz Hostile before open borders OFF for now
	...
	TeamTypes eTeam = plot()->getTeam();
	if ((GET_TEAM(getTeam()).isFriendlyTerritory(eTeam)) && !(GET_TEAM(GC.getGameINLINE().getActiveTeam()).isOpenBordersTrading()))
	{
	return false;
	}
...
{
	FAssertMsg(eInvisibleType >= 0, "eInvisibleType expected to be >= 0");
	...
	int iNewValue = getSeeInvisibleCount(eInvisibleType) + iChange;
	m_paiSeeInvisible[eInvisibleType] = iNewValue;
	FAssert(getSeeInvisibleCount(eInvisibleType) >= 0);
}

int CvUnit::getSeeInvisibleCount(InvisibleTypes eInvisibleType) const
{
	return m_paiSeeInvisible[eInvisibleType];
}
/**
** End: SeeInvisiblePromotion
**/
