...
void CvGameTextMgr::setPlotListHelp(CvWStringBuffer &szString, CvPlot* pPlot, bool bOneLine, bool bShort)
{
	...
	CvUnit* pLoopUnit;
//	static const uint iMaxNumUnits = 15; Vincentz plothelpbar
			...
			for (int iI = 0; iI < GC.getNumUnitInfos(); ++iI)
			{
				for (int iJ = 0; iJ < MAX_PLAYERS; iJ++)
				{
					...
					if (aiUnitNumbers[iIndex] > 0)
					{
						...
						{
//Vincentz plothelpbar		szString.append(L", ");
						}
						...
						{
							szString.append(CvWString::format(L"<img=%S size=25></img>", GC.getLeaderHeadInfo(GET_PLAYER((PlayerTypes)iJ).getLeaderType()).getButton()));
...
//Vincentz plothelpbar	szString.append(CvWString::format(SETCOLR L"%s" ENDCOLR, TEXT_COLOR("COLOR_UNIT_TEXT"), GC.getUnitInfo((UnitTypes)iI).getDescription()));
						szString.append(CvWString::format(L"<img=%S size=25></img>(%d) ", GC.getUnitInfo((UnitTypes)iI).getButton(), aiUnitNumbers[iIndex]));
...
//Vincentz Icon
					szString.append(CvWString::format(L"<img=%S size=64></img> vs. ", pAttacker->getButton()));
					szString.append(CvWString::format(L"<img=%S size=64></img> ", pDefender->getButton()));
					szString.append(NEWLINE);
//Vincentz Icon end
...
{
	int iI;
	szString.assign(L"!\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[x]^_`abcdefghijklmnopqrstuvwxyz\n");
	//szString.append(L"{}~\\ßÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÚÛÜÝÞŸßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿ¿¡«»°ŠŒŽšœž™©®€£¢”‘“…’"); Vincentz off
...
//			}
	...
	else if (bShift && !bAlt && (gDLL->getChtLvl() > 0))
	{
		...
		{
			/* ImmortalCulture: Replaced "Alive" with "EverAlive"
			   (only affects debug mode I think; plot help text with
			   shift key pressed) */
	...
	else
	{
		...
		if (eRevealOwner != NO_PLAYER)
		{
			...
			{
				szTempBuffer.Format(L"%d%% " , pPlot->calculateCulturePercent(eRevealOwner), GET_PLAYER(eRevealOwner).getPlayerTextColorR(), GET_PLAYER(eRevealOwner).getPlayerTextColorG(), GET_PLAYER(eRevealOwner).getPlayerTextColorB(), GET_PLAYER(eRevealOwner).getPlayerTextColorA(), GET_PLAYER(eRevealOwner).getCivilizationAdjective());
				szString.append(CvWString::format(L"<img=%S size=38></img>", GC.getCivilizationInfo(GET_PLAYER(eRevealOwner).getCivilizationType()).getButton()));
				szString.append(szTempBuffer);
//				szString.append(NEWLINE);
				...
				for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
				{
					if (iPlayer != eRevealOwner)
					{
						...
						{
							szTempBuffer.Format(L"%d%% " , pPlot->calculateCulturePercent((PlayerTypes)iPlayer), kPlayer.getPlayerTextColorR(), kPlayer.getPlayerTextColorG(), kPlayer.getPlayerTextColorB(), kPlayer.getPlayerTextColorA(), kPlayer.getCivilizationAdjective());
//							szTempBuffer.Format(L"%d%% " SETCOLR L"%s" ENDCOLR, pPlot->calculateCulturePercent((PlayerTypes)iPlayer), kPlayer.getPlayerTextColorR(), kPlayer.getPlayerTextColorG(), kPlayer.getPlayerTextColorB(), kPlayer.getPlayerTextColorA(), kPlayer.getCivilizationAdjective());
							szString.append(CvWString::format(L"<img=%S size=38></img>", GC.getCivilizationInfo(GET_PLAYER((PlayerTypes)iPlayer).getCivilizationType()).getButton()));
							szString.append(szTempBuffer);
//							szString.append(NEWLINE);
						}
					}
...
//Vincentz NOTE TerrainTypes 9 and next one 10 for hills, is the number of terrain in the Civ4TerrainInfo.xml. I couldnt find a better way to get the icon
				szString.append(CvWString::format(L"<img=%S size=38></img>", GC.getTerrainInfo((TerrainTypes)9).getButton()));
...
//					szString.append(gDLL->getText("TXT_KEY_PLOT_HILLS"));
					szString.append(CvWString::format(L"<img=%S size=38></img>", GC.getTerrainInfo((TerrainTypes)10).getButton()));
				}
...
//					szString.append(szTempBuffer);
					szString.append(CvWString::format(L"<img=%S size=38></img>", GC.getFeatureInfo(pPlot->getFeatureType()).getButton()));

				}

//				szString.append(GC.getTerrainInfo(pPlot->getTerrainType()).getDescription());
				szString.append(CvWString::format(L"<img=%S size=38></img>", GC.getTerrainInfo(pPlot->getTerrainType()).getButton()));
...
// Vincentz Start
		...
		{
			szString.append(CvWString::format(L"<img=%S size=38></img>", GC.getRouteInfo(pPlot->getRevealedRouteType(GC.getGameINLINE().getActiveTeam(), true)).getButton()));
		}
		...
		{
			szString.append(CvWString::format(L"<img=%S size=38></img>", GC.getImprovementInfo(eImprovement).getButton()));
		}
		if (GC.getGameINLINE().isDebugMode())
		{
			eBonus = pPlot->getBonusType();
...
//			szString.append(NEWLINE);
//			szString.append(szTempBuffer);
			szString.append(CvWString::format(L"<img=%S size=38></img>", GC.getBonusInfo(eBonus).getButton()));
...
//Vincentz #UPT		
	...
	if (pPlot->isVisible(GC.getGameINLINE().getActiveTeam(), GC.getGameINLINE().isDebugMode()))
	{
		...
		szTempBuffer.Format(L"Units (%d / %d) ", (iNumVisibleUnits), (GC.getDefineINT("MAXUPT") * GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getCurrentEra() + GC.getDefineINT("STARTUPT"))); //TODO
		szString.append(szTempBuffer);
...
//			szString.append(CvWString::format(L"<img=%S size=32></img>", GC.getImprovementInfo(eImprovement).getButton()));
 ...
 *
 ****************************************/
	for (iI = 0; iI < GC.getPromotionInfo(ePromotion).getNumSeeInvisibleTypes(); ++iI)
	{
		if (GC.getPromotionInfo(ePromotion).getSeeInvisibleType(iI) != NO_INVISIBLE)
		{
			szBuffer.append(NEWLINE);
			szBuffer.append(gDLL->getText("TXT_KEY_UNIT_SEE_INVISIBLE", GC.getInvisibleInfo((InvisibleTypes) GC.getPromotionInfo(ePromotion).getSeeInvisibleType(iI)).getTextKeyWide()));
		}
	}
/**
...
/*************************************************************************************************/
	...
	{
		szFirstBuffer = gDLL->getText("TXT_KEY_BUILDING_FROM_IN_ALL_CITIES", GC.getSpecialistInfo((SpecialistTypes) iI).getTextKeyWide());
		setCommerceChangeHelp(szHelpText, L"", L"", szFirstBuffer, GC.getCivicInfo(eCivic).getSpecialistCommerceChangeArray(iI));
	}
/*************************************************************************************************/
/**	CMEDIT: End																					**/
/*************************************************************************************************/
...
void CvGameTextMgr::setEspionageCostHelp(CvWStringBuffer &szBuffer, EspionageMissionTypes eMission, PlayerTypes eTargetPlayer, const CvPlot* pPlot, int iExtraData, const CvUnit* pSpyUnit)
{
	...
	if (kPlayer.getEspionageMissionCost(eMission, eTargetPlayer, pPlot, iExtraData, pSpyUnit) > 0)
	{
		...
		{
//Vincentz Spy	
			int iInterceptChance = (pSpyUnit->getSpyInterceptPercent(GET_PLAYER(eTargetPlayer).getTeam()) * (100 + kMission.getDifficultyMod() - pSpyUnit->evasionProbability())) / 100;
//Vincentz Spy
			szBuffer.append(NEWLINE);
			szBuffer.append(NEWLINE);
