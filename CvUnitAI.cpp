...
//	if ((getGroup()->getNumUnits() == 1) && (getDamage() > 0) &&
	if ((getGroup()->getNumUnits() == 1) && ((getDamage() > 0) && ((healRate(plot())*5) >= ((currHitPoints() * 100) / maxHitPoints()))) &&
...
//Vincentz Heal
	if ((getDamage() > 0) && ((healRate(plot())*5) >= ((currHitPoints() * 100) / maxHitPoints())))
	{
 ...
 *
 ****************************************/
	iTemp = GC.getPromotionInfo(ePromotion).getNumSeeInvisibleTypes();
...
bool CvUnitAI::AI_defend()
{
	...
	{
		//TODO get rangestrikes as defense Vincentz
		getGroup()->pushMission(MISSION_SKIP);
		return true;
	}
	...
	for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		...
		if (AI_plotValid(pLoopPlot))
		{
			if (potentialWarAction(pLoopPlot))
			{
				...
				if (pCity != NULL)
				{
//					if (pCity->isCoastal(GC.getMIN_WATER_SIZE_FOR_OCEAN())) Vincentz Blockade
//					{
		...
		if (AI_plotValid(pLoopPlot) || (pLoopPlot->isCity() && pLoopPlot->isAdjacentToArea(area())))
		{
			if (pLoopPlot->isOwned() && (pLoopPlot->getTeam() != getTeam()))
			{
				...
				while (pUnitNode != NULL)
				{
					...
					{
//						if (pLoopUnit->getDomainType() == DOMAIN_SEA && !pLoopUnit->isInvisible(getTeam(), false)) Vincentz Land Blockade
...
//										if (pRangePlot->isWater() && pRangePlot->isOwned() && isEnemy(pRangePlot->getTeam(), pLoopPlot)) Vincentz Land Blockade
										if (pRangePlot->isOwned() && isEnemy(pRangePlot->getTeam(), pLoopPlot))
										{
											bPlotBlockaded = true;
