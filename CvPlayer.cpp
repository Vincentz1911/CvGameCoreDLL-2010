...
void CvPlayer::init(PlayerTypes eID)
{
	...
	if ((GC.getInitCore().getSlotStatus(getID()) == SS_TAKEN) || (GC.getInitCore().getSlotStatus(getID()) == SS_COMPUTER))
	{
		...
		for (iI = 0; iI < GC.getNumUnitClassInfos(); ++iI)
		{
			...
			//Tholish commented this out so settlers would cost the set hammer cost ... Vincentz
			//			if (NO_UNIT != eUnit)
			//			{
			...
			if (!(pLoopUnit->isGoldenAge()))
			{
				if (pLoopUnit->getUnitInfo().getProductionCost() > 0)
				{
					if (!(pLoopUnit->isMilitaryHappiness()) || !(pLoopUnit->plot()->isCity()) || (pLoopUnit->plot()->plotCount(PUF_isMilitaryHappiness, -1, -1, getID()) > 1))
					{
						...
						//@MOD GFD: units inside cultural borders will be disbanded more likely (on strike)
...
void CvPlayer::findNewCapital()
{
	...
	eCapitalBuilding = ((BuildingTypes)GC.getDefineINT("CAPITAL_BUILDINGCLASS"));
	...
	if (pBestCity != NULL)
	{
		...
		//Vincentz No New Capital Palace
...
bool CvPlayer::splitEmpire(int iAreaId)
{
	...
	GC.getGameINLINE().updatePlotGroups();
	//Vincentz New Capital for Split Empire
	GET_PLAYER(eNewPlayer).findNewCapital();
