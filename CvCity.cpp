...
//Vincentz No unhealthy = 1/2 pop health instead of none
//	return 0;
	}
// Vincentz extra unhappiness from growth
	return std::max(0, ((getPopulation() * 2 + iExtra - ((bNoAngry) ? angryPopulation(iExtra) : 0))));
}
...
int CvCity::getSpecialistCivicExtraCommerceBySpecialist(CommerceTypes eIndex, SpecialistTypes eSpecialist) const
{
	...
	FAssertMsg(eSpecialist >= 0, "eSpecialist expected to be >= 0");
	FAssertMsg(eSpecialist < GC.getNumSpecialistInfos(), "GC.getNumSpecialistInfos expected to be >= 0");
	return ((getSpecialistCount(eSpecialist) + getFreeSpecialistCount(eSpecialist)) * GET_PLAYER(getOwnerINLINE()).getSpecialistCivicExtraCommerce(eSpecialist, eIndex));
}
...
//Vincentz Culture Less commerce in captured cities
...
int CvCity::countTotalCultureTimes100() const
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		// ImmortalCulture (v0.2): Replaced "Alive" with "EverAlive"
		if (GET_PLAYER((PlayerTypes)iI).isEverAlive())
		{
...
//Vincentz Better Vassal Cities
//					if (iValue > iBestValue)
					if ((iValue > iBestValue) && !(getOwnerINLINE() == (PlayerTypes)iPlayer))
					{
						iBestValue = iValue;
